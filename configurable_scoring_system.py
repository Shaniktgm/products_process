...
class ConfigurableScoringSystem:
    ...
    def __init__(self, config_file: str = "scoring_config.json"):
    ...
    def calculate_overall_score(self, product_data: Dict[str, Any], price_range: tuple = None) -> float:
        ...
        method = self.config.get("overall_score", {}).get("method", "overall_value_score")
        
        if method == "overall_value_score":
            ...
            # Default to overall_value_score
            return self._calculate_overall_value_score(product_data)
    ...
    def _calculate_weighted_composite_score(self, product_data: Dict[str, Any]) -> float:
        ...
        popularity_score = product_data.get('popularity_score', 0) or 0
        brand_reputation_score = product_data.get('brand_reputation_score', 0) or 0
        overall_value_score = product_data.get('overall_value_score', 0) or 0
        luxury_score = product_data.get('luxury_score', 0) or 0
        
        weighted_score = (
            total_score * weights.get('total_score', 0.3) +
            popularity_score * weights.get('popularity_score', 0.2) +
            brand_reputation_score * weights.get('brand_reputation_score', 0.2) +
            overall_value_score * weights.get('overall_value_score', 0.2) +
            luxury_score * weights.get('luxury_score', 0.1)
        )
    ...
    def calculate_sub_scores(self, product_data: Dict[str, Any]) -> Dict[str, float]:
        """Calculate sub-scores based on configuration"""
        sub_scores = {}
        
        for score_name, score_config in self.config.get("sub_scores", {}).items():
    ...
    def _calculate_fallback_score(self, score_name: str, method: str, product_data: Dict[str, Any]) -> float:
        """Calculate fallback score when CSV data is not available"""
        if method == "rating_based":
            rating = product_data.get('rating', 0) or 0
            return rating * 2 if rating else 5.0
    ...
    def apply_material_bonus(self, product_data: Dict[str, Any], base_score: float) -> float:
        """Apply material-based bonus to score"""
            ...
            with sqlite3.connect(db_path) as conn:
                ...
                for product_row in products:
                    try:
                        ...
                        # Calculate new overall score (using overall_value_score)
                        new_overall_score = self.calculate_overall_score(product_data, price_range)
                        
                        # Update database - only update overall_score column
                        cursor.execute("""
                            UPDATE products SET overall_score = ? WHERE id = ?
                        """, (new_overall_score, product_data['id']))
